...
void loop() {
  ...
  // Check if LoRa message was sent
  if (LoRa.beginPacket()) {
  ...
  if (loraMessageSent) {
    digitalWrite(ledPin, HIGH); // Turn on the LED when LoRa sends
  } else {
    digitalWrite(ledPin, LOW); // Turn off the LED when LoRa is not sending
  }
}
